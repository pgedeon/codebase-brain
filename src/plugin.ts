...
export default {
  ...
  register(api: OpenClawPluginApi) {
    const config = api.pluginConfig as { repoRoot: string; autoIndex?: boolean };
    ...
    const getHandler = async (): Promise<ToolHandler> => {
      if (!handler) {
    ...
    const ensureIndexed = async () => {
      if (!indexed) {
        ...
        try {
          ...
          // Table missing or other error - index anyway
          await performIndex();
          indexed = true;
        }
      }
